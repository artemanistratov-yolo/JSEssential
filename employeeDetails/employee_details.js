...
function displaybySpecialization() {
        const pythonemp = employees.filter(employee => employee.specialization === 'Python');
        const pythonempdisp = pythonemp.map((employee, index) => `<p>${employee.id}: ${employee.name} - ${employee.department} - $${employee.salary} - ${employee.specialization}</p>`).join('');
        document.getElementById('employeesDetails').innerHTML = pythonempdisp;
           }